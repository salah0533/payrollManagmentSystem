...
export const defaultPhoneCountryIso = "DZ";
export const defaultPhoneCountryCode = "+213";

export function getPhoneCountryByIso(iso2: string) {
  return phoneCountries.find((country) => country.iso2 === iso2) || phoneCountries.find((country) => country.iso2 === defaultPhoneCountryIso);
}

export function composePhoneNumber(countryIso: string, nationalNumber: string) {
  const countryCode = getPhoneCountryByIso(countryIso)?.dialCode || defaultPhoneCountryCode;
...
export function splitPhoneNumber(phone: string | null | undefined) {
  ...
  if (!match) {
    return { countryIso: defaultPhoneCountryIso, countryCode: defaultPhoneCountryCode, nationalNumber: normalized.replace(/^\+/, "") };
  }

  return {
    countryIso: match.iso2,
    countryCode: match.dialCode,
    nationalNumber: normalized.slice(match.dialCode.length),
  };
}
